...
@export var base_scroll_speed: float = 720.0 # Base speed of stripe movement
@export var amplitude_reduction_rate: float = 0.96 # Rate at which amplitude reduces
@export var speed_reduction_rate: float = 0.987 # Rate at which speed reduces
@export var width_increase_rate: float = 1.02 # Rate at which width increases
...
func _ready():
	var scroll_offset = (1 - speed_reduction_rate) + 1
	base_scroll_speed = get_viewport().size.y * scroll_offset
...
func _process(delta: float) -> void:
	...
	for stripe in stripes:
		...
		if stripe.position.y > get_viewport().size.y || stripe.amplitude < 0.0001:
